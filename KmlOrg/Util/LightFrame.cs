...
namespace Ops.NetCoe.LightFrame {
    ...
    public static class StdExtensions {
        ...
        /// <summary>
        /// Adds new element to the target according to the following rules:
        /// -If <paramref name="val"/> is <c>null</c> or empty then <paramref name="defaultVal"/> is used instead.
        /// - If <paramref name="defaultVal"/> is <c>null</c> then it won't be added at all and this method returns <c>null</c>.
        /// </summary>
        /// <param name="target"></param>
        /// <param name="elemName">Name of the new element</param>
        /// <param name="val">Value to add. Can be <c>null</c></param>
        /// <param name="defaultVal">Default value. If <c>null</c> then it will not be used.</param>
        /// <returns></returns>
        public static XElement AddElementIf(this XElement target, XName elemName, object val, string defaultVal=null) {
            ...
            XElement xn = new XElement(elemName, sval);
            target.Add(xn);
            return xn;
        }

        /// <summary>
        /// Tries to obtain value of the subelement <paramref name="elemName"/>.
        /// If such element found then <paramref name="tryApply"/> will be called.
        /// </summary>
        /// <param name="source"></param>
        /// <param name="elemName"></param>
        /// <param name="tryApply"></param>
        /// <returns></returns>
        public static bool TrGetElementVal(this XElement source, XName elemName, Action<string> apply) {
            var xe = source.Element(elemName);
        ...
        /// <summary>
        /// Get existing or create default XDocument for the specified <paramref name="xroot"/>.
        /// </summary>
        /// <param name="xroot"></param>
        /// <returns></returns>
        public static XDocument EnsureXDoc(this XElement xroot) {
            if (xroot.Document==null) {
                return new XDocument(new XDeclaration("1.0", "UTF-8", null), xroot);
            }
            else {
                return xroot.Document;
            }
        }

        public static string ToStringWithDeclaration(this XDocument xdc, Encoding encoding=null) {
            StringBuilder sb = new StringBuilder();
            using(var swr= new StringWriterWithEncoding(encoding??Encoding.UTF8, sb)) {
                xdc.Save(swr);
            }
            return sb.ToString();
        }
